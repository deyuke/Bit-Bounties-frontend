...
export const useBounties = () => {
    ...
    useEffect(() => {
        axios
          ...
          .then((res) => {
            setCompanyData(
              postings.map(bounties => 
                res.data.find((company: Company) => company.id === bounties.company_id)
      ...
      useEffect(() => {
        if (companyData) {
          const combinedData = postings.map(bounties => {
            const company = companyData.find(company => company.id === bounties.company_id);
            return {
              ...bounties,
